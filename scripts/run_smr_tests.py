...
{{
	...
	"snp_list_file":
                "/users/mgloud/projects/coloc_comparisons/tmp/smr_snp_list.txt",
...
for i in range(num_tests):
    ...
    with open("/users/mgloud/projects/coloc_comparisons/tmp/smr_snp_list.txt", "w") as w:
        w.write("{0}\t{1}\n".format(answers[i]["seed_chrom"], answers[i]["seed_pos"]))

    # Dump the config template to a file
    with open("/users/mgloud/projects/coloc_comparisons/tmp/smr.config", "w") as w:
        w.write(config_template.format(i, base_dir, base_last_dir))

    # Get it going
    subprocess.call("python /users/mgloud/projects/brain_gwas/scripts/dispatch.py /users/mgloud/projects/coloc_comparisons/tmp/smr.config 1", shell=True)
