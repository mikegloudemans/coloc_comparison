...
def main():
    ...
    subprocess.call("rm -rf /users/mgloud/projects/brain_gwas/output/finemap-three-causal/*", shell=True)
    ...
    for i in range(len(kept_data)):
        ...
        #while int(subprocess.check_output('''ps -ef | grep "python /users/mgloud/projects/brain_gwas/scripts/dispatch.py /users/mgloud/projects/gwas/tmp/lc_config" | grep -v grep | wc -l''', shell=True)) > 0:
            time.sleep(1)

template = '''
{
        "out_dir_group": "finemap-three-causal",
