...
def main():
    ...
    subprocess.call("rm -rf /users/mgloud/projects/brain_gwas/output/some-locus-compare-tests-with-coloc/*", shell=True)
    ...
    for i in range(len(kept_data)):
        ...
        while int(subprocess.check_output('''ps -ef | grep "python /users/mgloud/projects/brain_gwas/scripts/dispatch.py /users/mgloud/projects/gwas/tmp/lc_config" | wc -l''', shell=True)) > 8:
            time.sleep(5)

template = '''
{
        "out_dir_group": "some-locus-compare-tests-with-coloc",
