...
def main():
    ...
    {{
            ...
            "snp_list_file":
                    "/users/mgloud/projects/coloc_comparisons/tmp/gsmr_snp_list{0}.txt",
    ...
    subprocess.call("rm -rf /users/mgloud/projects/brain_gwas/output/gsmr-comparisons/{0}".format(base_last_dir), shell=True)
...
def run_test(i, answers, config_template, base_dir, base_last_dir):
        ...
        with open("/users/mgloud/projects/coloc_comparisons/tmp/gsmr_snp_list{0}.txt".format(i), "w") as w:
            w.write("{0}\t{1}\n".format(answers[i]["seed_chrom"], answers[i]["seed_pos"]))

        # Dump the config template to a file
        with open("/users/mgloud/projects/coloc_comparisons/tmp/gsmr{0}.config".format(i), "w") as w:
            w.write(config_template.format(i, base_dir, base_last_dir))

        # Get it going
        subprocess.call("python /users/mgloud/projects/brain_gwas/scripts/dispatch.py /users/mgloud/projects/coloc_comparisons/tmp/gsmr{0}.config 1".format(i), shell=True)
